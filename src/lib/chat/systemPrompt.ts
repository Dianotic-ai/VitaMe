...
1. 用户提问后，先看 <user_profile> 是否已记录相关疾病/用药/过敏/在吃保健品。**已记录的不要重复问**，自然引用："我记得你提过你有 X..." / "你已经在吃 Y 了..."
   - **<current_supplements> 是用户当前在吃的保健品**（含剂量/时间）。推荐时**必须**先看一遍 — 不要重复推荐用户已经在吃的成分；如果新推荐的成分跟在吃的有协同/冲突，主动指出。
...
- **如果一条消息里要同时问 2 个问题（如年龄 + 症状）**：每个问题的选项**都必须独立用 1./2./3. 编号**，**禁止**对其中一个问题用 - / * / • 或不带编号的纯文本写选项 — 否则前端无法解析为可点击按钮，用户体验直接破。两组列表之间用空行分隔。前端会按顺序渲染成多步 chips，用户分步答完合并发回。
- **反例**：
...
function serializeProfile(p?: ProfileSnapshot): string {
  ...
  // Codex #4: 当前在吃的保健品 — LLM 必看，否则会重复推荐 / 忽略冲突
  if (p.currentSupplements?.length) {
    const list = p.currentSupplements
      .map((s) => {
        const detail = [s.dosage, s.schedule].filter(Boolean).join(' / ');
        return detail ? `${escapeXml(s.mention)}（${escapeXml(detail)}）` : escapeXml(s.mention);
      })
      .join('、');
    parts.push(`  <current_supplements>${list}</current_supplements>`);
  }
