...
interface MemoryEventInput {
  /** 客户端真实 eventId — Hermit observation.basedOnEventIds 必须从这里挑 */
  eventId: string;
  eventType: string;
  occurredAt: string;
  entityRefs: string[];
...
export interface ObservationOutput {
  ...
  /** 提案（可执行的小行动） */
  proposal?: string;
  /** 这条 observation 基于哪些 event（让用户知道为什么）— Codex #6 后必填 */
...
每条 observation 形如：
{
  ...
  "basedOnEventIds": ["这条观察基于的具体 eventId（≥1 个，从 <events> 输入里的 eventId 字段照搬，禁止造）"]
}

# basedOnEventIds 铁律
- 必须 ≥1 个 eventId，且每个 id 必须**字面照搬** <events> 输入里的 eventId 值
- 禁止编造 / 截断 / 重组 id
- 服务端会过滤掉不在输入里的 id，全部不匹配的 observation 会被丢弃
...
function sanitize(raw: HermitRawOutput[], validEventIds: Set<string>): ObservationOutput[] {
  ...
  return raw
    ...
    .map((r) => {
      // Codex Finding #6: basedOnEventIds 必须 ⊆ 输入 eventIds，否则不可追溯
      const filteredIds = Array.isArray(r.basedOnEventIds)
        ? r.basedOnEventIds.filter((id) => typeof id === 'string' && validEventIds.has(id)).slice(0, 10)
        : [];
      return {
        ...
        basedOnEventIds: filteredIds,
        entityRefs: Array.isArray(r.entityRefs) ? r.entityRefs.slice(0, 5) : undefined,
      };
    })
    // 没有任何有效 basedOnEventIds 的 observation 直接丢弃 — 不可追溯就不展示
    .filter((o) => o.basedOnEventIds.length > 0)
    .slice(0, 3);
}
...
输出 JSON 数组：`;
  ...
  try {
    ...
    const validIds = new Set(recentEvents.map((e) => e.eventId).filter((id) => typeof id === 'string'));
    const observations = sanitize(tryParseJson(text), validIds);
