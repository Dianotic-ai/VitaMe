...
## 3. Vercel Template Lessons

Vercel Vibe Coding Platform 可作为 Agent engineering reference，不作为 VitaMe 产品底座。它的 text-to-app、sandbox 和项目生成能力不进入 VitaMe P0；可借鉴的是 AI Gateway、streaming UX、tool trace 和 provider abstraction。

VitaMe 的 P0 Agent Shell 分层：
...
Next.js Chat UI
 │
 ▼
Vercel AI SDK Agent Shell
 │
 ├── Model Provider Layer
 │     └── AI SDK provider abstraction / AI Gateway
 │
 ├── Tool Loop Layer
 │     ├── parseIntentTool
 │     ├── runJudgmentTool
...
Key adaptation rule：VitaMe 使用 Vercel-style Agent shell 来展示工具调用过程，但不使用 text-to-app 叙事。Demo 必须表达“健康安全 Agent 调用可信工具”，不是“AI 生成健康应用”。
