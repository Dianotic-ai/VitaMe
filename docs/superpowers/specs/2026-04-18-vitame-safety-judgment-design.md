...
- **RuleRegistry** (`src/lib/judgment/ruleRegistry.ts`):载入 `src/lib/db/contraindications.ts` 的 ~50 条硬编码规则,按 `(ingredient, condition|medication)` 建索引。查询 O(1)。每条规则形如 `{ key, level, reason_code, source: "hardcoded", source_ref: "VitaMe-rule-<id>" }`。
...
- **JudgmentEngine** (`src/lib/judgment/judgmentEngine.ts`):pipeline 总入口。`Promise.all([hardcoded, suppai, ddinter])` 并发 3 路,聚合结果。超时阈值 500ms(离线查找通常 <10ms,这个阈值仅作兜底)。
...
- **Ingredient 不在任何数据库/规则**:level = `gray`(灰色,"暂无数据"),`reason_code = "no_data"`,`source_type = "limited"`;**不静默回绿色**,Compliance 层 UI 必须显示"有限证据"徽章。
- **多 Ingredient 触发多个 red**:全部列出,`overallLevel = red`,不合并成笼统"多重禁忌"。用户需要看到每一条的具体原因。
- **SUPP.AI Map 未命中 (ingredient × medication) 组合**:视为"无已知风险",level = `green`,`reason_code = "no_known_interaction"`;前端展示仍附 `source: "SUPP.AI"` 证据来源(表示我们查了)。
...
场景 5 — 3 路 adapter 全部空(冷门成分):

- WHEN `judge({ ingredients: ["黄金燕窝肽"], medications: [] })`
- THEN `HardcodedAdapter` 未命中 → `[]`
- AND `SuppaiAdapter` Map 未命中 → `[]`
- AND `DdinterAdapter` 返回 `[]`
- AND `RiskLevelMerger` 产出 `{ level: "gray", reason_code: "no_data", source_type: "limited" }`
- AND `overallLevel = "gray"`
- AND Compliance 层 UI 标注"有限证据"(参见 compliance-design 场景 3)
