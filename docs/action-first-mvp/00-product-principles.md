...
### 4.7 Visualization as differentiation

VitaMe 的差异化不只在 prompt 工程或安全规则，**也在视觉**。具象药盒（早 / 中 / 晚 / 睡前 4 格 = 4 颗等待发芽的种子）+ "种子→发芽→开花→结果" 四阶段图形系统，是产品的 signature。

- 药盒 + 种子是双 signature 物件，承载"判断之后的生长"隐喻。
- 每个新页面、新组件、新空状态必须考虑：是否出现 signature、出现在哪、是否破坏隐喻。
- 不靠文案解释 signature，靠图形传达。
- 这条原则不可被"功能优先""信息密度""简化空状态"等理由砍。删 signature = 削弱产品记忆点，需 Kevin 显式同意。

视觉契约见 `docs/product/品牌视觉规范.md` §11 Signature Objects 与 `DESIGN.md` §11.1（种子四阶段）+ §11.5（药盒）。
