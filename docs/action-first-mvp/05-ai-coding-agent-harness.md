...
- 改变 P0→P1 唯一通道。
- 改免责声明和医疗边界。
- 把 Product Inspect 的临时结果自动写成用户长期 memory。
- 让 `safetyMemory` 在 P0 请求里出现。
...
- crawl4ai 不可用或超时时，URL 流程仍能降级返回。
- 单纯粘贴 URL 不写 actionMemory。
...
- 保存前 actionMemory 为空。
- 保存后顶部条出现。
- 清空 routine 后 `/api/chat` 不带 safetyMemory。
...
| 文件/模块 | 必须做 | 禁止做 |
|---|---|---|
| `src/app/chat/page.tsx` | 移除 PersonSwitcher 主路径；发送 P0 `{sessionId,messages}`；接入 RoutineConfirmCard、RoutineSummaryStrip、URL inspect trigger | 不发送 `personToSnapshot(activePerson)`；不在每轮结束后调用 `/api/extract` |
| `src/app/api/chat/route.ts` | 在 retrieve/LLM 前执行 red flag pre-check；支持 P1 compact `safetyMemory` | 不接受完整 `profile`；不序列化 `<user_profile>` |
| `src/app/api/extract/route.ts` | 删除或使其不可达 | 不保留 post-turn profile extractor |
| `src/lib/profile/*` | MVP 主路径不引用；旧数据仅作为遗留数据存在 | 不迁移为 Action-First 状态源；不创建多人档案 |
| `src/lib/safety/redFlags.ts` | deterministic regex；返回 hard route 文案 | 不依赖 LLM 判断高危 |
| `src/app/api/product/inspect/route.ts` | URL/text inspect；错误码；fallback | 不做购物车批量解析；不做价格/渠道推荐 |
| `src/lib/productInspect/*` | parser fixture；fetch/parser；可选 crawl4ai helper | crawl4ai 不可成为必需依赖 |
| `src/lib/routine/*` | LocalStorage routine；add/clear/check | 不由 LLM tool-call 静默保存 |
| `src/lib/actionMemory/*` | 只写 explicit action event | 不写 conditions/age/sex/summary/person |
| `tests/unit/*` | redFlags、product parser、routine parser、actionMemory tests | 不只做 snapshot 测试 |
